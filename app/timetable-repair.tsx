...
export default function TimetableRepairScreen() {
    ...
    const [status, setStatus] = useState('Ready to run same-day existing timetable conflict repair.');

    const runRepair = () => {
        setLoading(true);
        setStatus('Running same-day period reshuffle across affected class and teacher timetables...');
        repairTimetable(generatedBatchId)
            .then(data => { setResult(data); setStatus(data.publishReady ? 'Same-day repair complete. Timetable is ready to publish.' : 'Same-day repair attempted. Review remaining conflicts in Web ERP.'); })
    ...
    return <ImageBackground source={require('../assets/branding/splash-gold.png')} style={styles.bg} resizeMode="cover"><ScrollView contentContainerStyle={styles.container}>
        <Header title="Auto Conflict Repair" eyebrow="SAME-DAY REPAIR ENGINE" homePath={backHome} />
        ...
        <View style={styles.card}><Text style={styles.cardTitle}>Same-Day Reshuffle Actions</Text><Text style={styles.cardNote}>Existing timetable repair preserves teacher, subject, and class ownership. It only reshuffles period positions on the same day where a valid improvement is found.</Text>{(result?.actions || ['Tap Run Auto Repair to resolve existing timetable conflicts using same-day period reshuffling.']).map((item, index) => <Text key={index} style={styles.action}>• {item}</Text>)}</View>
        <TouchableOpacity style={styles.primaryButton} onPress={runRepair}><Text style={styles.primaryText}>Run Same-Day Auto Repair Again</Text></TouchableOpacity>
