...
export default function StudentDashboard() {
    ...
    const topCenterTitle = useMemo(() => {
        if (selectedAttendance === 'TODAY') return 'Today Subject-wise Attendance';
        if (selectedAttendance === 'WEEKLY') return 'Weekly Subject-wise Attendance';
        if (selectedAttendance === 'MONTHLY') return 'Monthly Subject-wise Attendance';
        if (examDetail) return examResults[examDetail].title;
        ...
        <ImageBackground source={backgroundSource} style={styles.background} resizeMode="cover">
            <ScrollView contentContainerStyle={styles.container}>
                <View style={styles.topRow}>
                    <TouchableOpacity style={styles.circleButton} onPress={openMenu} activeOpacity={0.85}>
                        <Text style={styles.circleButtonText}>☰</Text>
                    </TouchableOpacity>

                    <Text style={[styles.topCenterTitle, isHomePage && styles.homeTopTitle]}>
                        {topCenterTitle}
                    </Text>
                ...
                {selectedAttendance ? (
                    contentReady ? (
                        <AttendanceDetail view={selectedAttendance} onBack={closeAttendance} />
                    ) : (
                    ...
                    examDetail ? (
                        contentReady ? (
                            <ExamDetailScreen type={examDetail} onBack={() => setExamDetail(null)} />
                        ) : (
                            <LoadingPanel label="Loading exam details..." />
                        )
                    ) : (
                        <ExamResultsScreen
                            onOpenRecent={() => setExamDetail('RECENT')}
                            onOpenPast={() => setExamDetail('PAST')}
                            onBack={goHome}
                        />
                    )
                    ...
                    <HomeScreen studentName={studentName} onOpenAttendance={openAttendance} onLogout={goLogin} />
                )}
            </ScrollView>
    ...
    return (
        <>
            ...
            <View style={styles.heroCard}>
                ...
                <Text style={styles.heroText}>Class 10 • Section A</Text>
            ...
            <View style={styles.grid}>
                <AttendanceCard title="Today" value="Present" hint="Tap to view graph" onPress={() => onOpenAttendance('TODAY')} />
                <AttendanceCard title="Weekly" value="5 / 6 Days" hint="Tap to view graph" onPress={() => onOpenAttendance('WEEKLY')} />
                <AttendanceCard title="Monthly" value="21 / 24 Days" hint="Tap to view graph" onPress={() => onOpenAttendance('MONTHLY')} />
...
const AttendanceDetail = memo(function AttendanceDetail({
    ...
    const maxValue = useMemo(() => Math.max(...data.map((item) => item.present + item.absent)), [data]);
            ...
            <View style={styles.chartCard}>
                ...
                <View style={styles.barChartArea}>
                    {data.map((item) => {
                        ...
                        const absentHeight = Math.max(item.absent === 0 ? 0 : 14, (item.absent / maxValue) * 150);

                        return (
                            <View key={item.subject} style={styles.barGroup}>
                                <View style={styles.barsWrapper}>
                                    <View style={[styles.presentBar, { height: presentHeight }]} />
                                    <View style={[styles.absentBar, { height: absentHeight }]} />
                                </View>
                                <Text style={styles.barLabel} numberOfLines={2}>{item.subject}</Text>
                            </View>
                        );
                    })}
                </View>
            ...
            {data.map((item) => (
                <View key={item.subject} style={styles.subjectRow}>
                    <View>
                        ...
                        <Text style={styles.subjectSubText}>Present: {item.present} | Absent: {item.absent}</Text>
                    </View>

                    <Text style={[styles.statusText, item.status === 'Absent' && styles.absentText]}>
                        {item.status}
                    </Text>
                </View>
            ))}
...
const ExamResultsScreen = memo(function ExamResultsScreen({
    ...
    return (
        <>
            <TouchableOpacity style={styles.card} onPress={onOpenRecent} activeOpacity={0.85}>
            ...
            <TouchableOpacity style={styles.card} onPress={onOpenPast} activeOpacity={0.85}>
        ...
        <>
            ...
            {result.subjects.map((item) => (
                <View key={item.subject} style={styles.subjectRow}>
                    <View>
                        <Text style={styles.subjectName}>{item.subject}</Text>
                        <Text style={styles.subjectSubText}>Marks: {item.marks}</Text>
                    </View>
...
const SchoolNoticesScreen = memo(function SchoolNoticesScreen({ onBack }: { onBack: () => void }) {
    return (
        <>
...
const SummaryBox = memo(function SummaryBox({ title, value }: { title: string; value: string }) {
    return (
...
const MenuItem = memo(function MenuItem({ title, onPress }: { title: string; onPress: () => void }) {
    return (
