...
export default function TimetableReviewScreen() {
    ...
    useEffect(() => {
        if (!generatedBatchId || generatedBatchId === 'DEMO' || generatedBatchId.startsWith('TT-DEMO')) return;
        let active = true;
        setLoading(true);
...
function PageHeader({ title, eyebrow, homePath }: { title: string; eyebrow: string; homePath: string }) { return <MobileWorkflowHeader title={title} eyebrow={eyebrow} homePath={homePath} />; }
